  ...
  return (
    <div className="page-stack">
      ...
      <ScreenHeader
        eyebrow="Expenses"
        title="支出"
        description="保存済み支出の集計と、登録履歴への入口をまとめています。削除は取り込み単位で行えます。"
      />
      ...
      <SectionCard
        title="支出レポート"
        description="月ごとの日別支出を横棒グラフで確認できます。気になる日付はそのまま詳細へ進めます。"
      >
        <Link className="link-card link-card-featured" href="/expenses/reports">
          <strong>支出レポート</strong>
          <span>月別の推移と、日ごとの金額の偏りを確認できます。</span>
        </Link>
      </SectionCard>

      <SectionCard
        title="登録履歴"
        description="保存済み支出を日ごとに振り返れる履歴画面です。必要ならその日の詳細へ進めます。"
      >
        <Link className="link-card" href="/expenses/history">
          <strong>登録履歴を開く</strong>
          <span>日付単位で合計を確認しながら、過去の登録内容をたどれます。</span>
        </Link>
      </SectionCard>
          ...
          {snapshot.groups.length === 0 ? (
            <div className="empty-state">
              ...
              <p className="section-copy">
                手入力や読み取り結果の保存を行うと、ここに登録履歴として表示されます。
              </p>
              <div style={{ height: 14 }} />
            ...
            snapshot.groups.map((group) => (
              <section className="expense-card" data-testid="expenses-group" key={group.importGroupId}>
                <div className="expense-card-main">
                  <div className="expense-card-overline">
                    <span className="pill pill-accent">{formatSourceLabel(group.sourceType)}</span>
                    <span className="expense-card-date">{formatDisplayDate(group.occurredOn)}</span>
                  </div>

                  <div className="expense-card-header">
                    <div>
                      <p className="expense-group-label">{formatImportGroupHeading(group.sourceType)}</p>
                      <p className="list-title">{group.merchantName ?? "店舗名なし"}</p>
                      <p className="list-meta">
                        {formatImportGroupMetaLabel(group.sourceType)}: {formatSourceLabel(group.sourceType)}
                      </p>
                    </div>
                    <strong className="expense-amount">{formatCurrency(group.totalAmount)}</strong>
                  </div>

                  <div className="expense-summary-grid">
                    <div className="expense-summary-card">
                      <span className="expense-summary-label">明細数</span>
                      <strong className="expense-summary-value">{group.itemCount}件</strong>
                    </div>
                    <div className="expense-summary-card">
                      <span className="expense-summary-label">合計金額</span>
                      <strong className="expense-summary-value">{formatCurrency(group.totalAmount)}</strong>
                    </div>
                  </div>
