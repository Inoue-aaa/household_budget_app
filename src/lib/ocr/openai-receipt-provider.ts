...
const RECEIPT_SYSTEM_PROMPT =
  "You analyze Japanese receipt images and return structured JSON for a household budget app. Distinguish item rows, discount-like rows, subtotal, tax, total, tendered amount, change, and unknown rows. Treat rows containing 会員様割引, 会員値引, 値引, 割引, 特売, クーポン, or coupon as discount-like rows. If a discount-like row clearly belongs to the nearest previous product row, merge it into that product instead of emitting a separate item. Use discountAmount as a negative number and finalAmount = originalAmount + discountAmount. If the linkage is unclear, do not force it into item; place it in unknownRows or mark it unknown. Never classify tendered or change as item. Keep merchantName, occurredOn, rawText, and product names as close to the receipt text as possible. Do not replace product names with generic labels. categoryHint should be conservative: if unsure, use other. Do not classify masks, tissues, detergent, batteries, stationery, wrap, or trash bags as food.";

const RECEIPT_USER_PROMPT =
  "Read these Japanese receipt images and return JSON only. Return merchantName, occurredOn, items[], and unknownRows[] when helpful. For each item row, you may include name, rawText, quantity, unitPrice, originalAmount, discountAmount, finalAmount, categoryHint, and confidence. Prefer finalAmount when you can determine the discount-adjusted amount. Keep product names close to the receipt text and never replace them with generic words. categoryHint should be conservative: food is only for clear food and drink items such as 弁当, パン, 菓子, 野菜, 肉, 魚, 飲料. daily_goods is for マスク, ティッシュ, 洗剤, トイレットペーパー, 石鹸, 電池, 文具, ラップ, ゴミ袋. medicine is for 薬, 風邪薬, 鎮痛剤, 胃薬. beauty is for シャンプー, 化粧品, スキンケア, ヘアケア. clothing is for シャツ, 靴下, インナー. If unclear, use other. Do not classify masks or other obvious non-food items as food. If the product name is too short or ambiguous, use other. For each row, set kind to one of item, subtotal, tax, total, tendered, change, or unknown. Never label tendered or change as item. If a row is unclear, use unknown or unknownRows instead of forcing it into item.";
